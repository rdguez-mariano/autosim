...
class IMAS_keypoints(object):
    ...
    def GetSimulatedKPs(self,idx):
        ...
        if np.shape(idx)==():
            ...
            idx = np.ravel_multi_index([x,y], self.img.shape)
        simulated_kps = []
        for idxs in self.imasdetected_idxs[idx]:
            i,j = idxs
            simulated_kps.append( AffineKPcoor(self.kplists[i][j:j+1], self.A_simu_to_orig[i])[0] )
        return simulated_kps
    def GetOrderedImasKpIdxs(self):
        idxs = np.flip(np.argsort(self.imasdetected, axis=None))
        return idxs
    def Idx2Pt(self, idx):        
        # needs the flip because kpt.pt = y,x
        return np.flip(np.unravel_index(idx, self.imasdetected.shape))
    def Prepare2Use(self, Rooted = True):
        N = np.sum([len(kps) for kps in self.kplists])
        # self.desc = np.zeros( shape=(N,self.descs[0].shape[1]), dtype = self.descs[0].dtype )
        countD = 0
        for i in range(len(self.kplists)):
            KPlist1_in_orig = AffineKPcoor(self.kplists[i],self.A_simu_to_orig[i])
            in_all_idx1 = []
            siftdetected = np.zeros(np.shape(self.img), dtype=np.float32)
            for j in range(len(KPlist1_in_orig)):
                self.kplist.append( KPlist1_in_orig[j] )
                in_all_idx1.append( len(self.kplist)-1 )
                self.simu_idx.append( [i,j] )
                y, x = np.round(KPlist1_in_orig[j].pt).astype(int)
                siftdetected[x,y] += 1.0
                self.imasdetected_idxs[y+np.shape(self.img)[1]*x].append( [i,j] )
            self.simu2orig_detected.append(siftdetected)
            self.in_all_idx.append( in_all_idx1 )
            if self.SIFTAID:
                for k in range(len(self.descs[i])):
                    self.desc[countD+k,:] = self.descs[i][k,:]
                countD += len(self.descs[i])
            else:
                # self.desc[countD:(countD+self.descs[i].shape[0]),:] = self.descs[i][:,:]
                # countD += self.descs[i].shape[0]  
                pass              
            self.imasdetected[siftdetected>0.0] += 1.0
            # self.imasdetected += siftdetected
        self.imasdetected /= len(self.kplists)
        # if Rooted and (not self.SIFTAID):
        #     self.desc = np.sqrt(sklearn.preprocessing.normalize(self.desc, norm='l2',axis=1))
    
    def Compute_IMAS_Keypoints_from_image(self, tilts = tilts_1_7, SIFTAID = True):
        self.SIFTAID = SIFTAID
        img = self.img
        h, w = img.shape[:2]
        for i in range(int(len(tilts)/2)):
            tilt = tilts[2*i]
            for theta in np.arange(0.0,np.pi,tilts[2*i+1]):
                img1, mask1, A1, Ai1 = SimulateAffineMap(1.0, 0.0, tilt, theta, img, inter_flag = cv2.INTER_LINEAR, border_flag=cv2.BORDER_CONSTANT)
                # noise = cv2.randu(np.zeros(shape=img1.shape), 0, 255)
                # img1[mask1==0] =  noise[mask1==0]
                maxoctaves = 6
                pyr1 = buildGaussianPyramid( img1, maxoctaves+2 )
                sift_des1 = []
                if SIFTAID:
                    KPlist1 = ComputeSIFTKeypoints(img1, MaxNum = -3000)
                    # KPlist1, _ = Filter_Affine_In_Rect(KPlist1,A1,[0,0],[w,h],isSIFT=True)
                    KPlist1, _ = Filter_Affine_In_Rect(KPlist1,A1,[0,0],[w,h],isSIFT=False)
                    patches1 = []
                    if len(KPlist1)>0:
                        patches1, A_list1, Ai_list1 = ComputePatches(KPlist1,pyr1, border_mode=cv2.BORDER_CONSTANT)                
                        ...
                        sift_des1 = train_model.get_layer("aff_desc").predict(bP)
                    self.patches.append( patches1 )
                else:
                    # KPlist1, sift_des1 = ComputeSIFTKeypoints(img1, Desc = True, MaxNum = -1000)
                    # KPlist1, sift_des1,_ = Filter_Affine_In_Rect(KPlist1,A1,[0,0],[w,h], desc_list=sift_des1,isSIFT=True)
                    sift_des1 = []
                    KPlist1 = ComputeSIFTKeypoints(img1, MaxNum = -1000)
                    KPlist1, _ = Filter_Affine_In_Rect(KPlist1,A1,[0,0],[w,h], isSIFT=True)
                
                self.A_orig_to_simu.append( A1 )
                self.A_simu_to_orig.append( Ai1 )
                self.kplists.append( KPlist1 )
                self.descs.append( sift_des1 )
                self.pyrs.append( pyr1 )
                self.tilts.append( [tilt, theta] )
                self.imgs.append( img1 )
                self.masks.append( mask1 )
                # WriteImgKeys(img1, KPlist1, 'im1/'+str(len(self.kplists))+'.png')
                # WriteImgKeys(mask1, KPlist1, 'mask1/'+str(len(self.kplists))+'.png')
        self.Prepare2Use()
...
def WriteImgKeys(img, keys):
    colors=( (0, 0, 255), (0, 255, 0), (255, 0, 0), (255, 255, 0), (0, 255, 255) )
    # patch = cv2.cvtColor( img.astype(np.uint8), cv2.COLOR_GRAY2RGB )
    for n in range(np.min([len(colors),len(keys)])):
        img=cv2.drawKeypoints(img,keys[n:n+1],img, color=colors[n] ,flags=0)
    return img
...
class CPPbridge(object):
    ...
    def GeometricFilter(self, KPlist1, im1, KPlist2, im2, Matches, Filter = 'ORSA_H', precision = 24, verb=False):
        ...
        self.libDA.GeometricFilter(src_pts.ctypes.data_as(floatp), dst_pts.ctypes.data_as(floatp),
                                    ...
                                    ctypes.c_int(N), ctypes.c_int(w1), ctypes.c_int(h1), ctypes.c_int(w2), ctypes.c_int(h2), ctypes.c_int(filtercode), ctypes.c_float(precision), verb)
        Consensus = []
    ...
    def GeometricFilterFromMatcher(self, im1, im2, Filter = 'ORSA_H', precision=24, verb=False):
        ...
        self.libDA.GeometricFilterFromNodes(self.MatcherPtr, T.ctypes.data_as(floatp),
                                    ctypes.c_int(w1), ctypes.c_int(h1), ctypes.c_int(w2), ctypes.c_int(h2),
                                    ctypes.c_int(filtercode), ctypes.c_float(precision), ctypes.c_bool(verb))
    ...
    def CreateMatcher(self,desc_dim, k=1, sim_thres=0.7):
        self.MatcherPtr = self.libDA.newMatcher(ctypes.c_int(k),ctypes.c_int(desc_dim),ctypes.c_float(sim_thres))
